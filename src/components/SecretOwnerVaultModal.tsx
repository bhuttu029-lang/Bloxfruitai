...
export const SecretOwnerVaultModal: React.FC<SecretOwnerVaultModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (isAuth) {
        ...
        loginOwnerWithServer(initialPrefillKey, '4770').then(res => {
          if (res.success) {
            setIsAuthenticated(true);
            reloadData();
          }
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    const effectivePreAuth = cleanStep1 || (cleanKey.toLowerCase().startsWith('4770') ? '4770' : undefined);
    ...
    if (res.success) {
      ...
      setAuthError(res.error || '⛔ Access Denied. Pre-authorization code (4770) and Master Key (bhuttu4770) required.');
      soundFX.playPop();
    }
  };
          ...
          {!isAuthenticated ? (
            <div className="max-w-md mx-auto py-8 text-center space-y-6">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="Enter pre-authorization code (e.g. 4770)..."
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="Enter master clearance key (e.g. bhuttu4770)..."
              ...
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 text-[11px] text-slate-500 text-left space-y-1">
                <p>🔒 <strong>Zero-Trust Protocol:</strong> Entering Step 1 (<code className="text-amber-300">4770</code>) followed by Step 2 (<code className="text-cyan-300">bhuttu4770</code>) validates owner credentials with cryptographic constant-time verification.</p>
              </div>
            </div>
          ) : (
