...
export const AiOracleChat: React.FC<AiOracleChatProps> = ({ currentTrade, initialQuery }) => {
  ...
  const handleSend = async (queryText?: string) => {
    ...
    // 0a. Step 1 of Mandatory Owner Protocol: "4770" (Silent Arming)
    const isStep1Input = lower === '4770' || lower === '/4770' || lower === 'code 4770' || lower === 'code: 4770' || lower === 'code:4770' || clean === '4770';
    if (isStep1Input) {
      ownerStep1TimeRef.current = Date.now();
      ownerStep1TextCountRef.current = 0;
      armOwnerSequenceOnServer('4770').catch(() => {});
    ...
    // 0b. Step 2 of Mandatory Owner Protocol: "bhuttu4770" or combined sequence "4770bhuttu4770"
    const isMasterKeyInput = lower === 'bhuttu4770' || lower === '/bhuttu4770' || lower === '/owner bhuttu4770' || lower === 'owner bhuttu4770' || lower === 'owner:bhuttu4770' || lower === 'owner: bhuttu4770' || clean === 'bhuttu4770';
    const isCombinedFullKey = lower === '4770bhuttu4770' || lower === '4770 bhuttu4770' || lower === '/owner 4770bhuttu4770' || lower === '/vault 4770bhuttu4770';
    ...
    if (isMasterKeyInput || isCombinedFullKey) {
      ...
      if (isSequenceArmed || isCombinedFullKey) {
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('blox_fruits_open_owner_vault', { detail: { prefillKey: 'bhuttu4770' } }));
        }

        soundFX.playWin();

        // Notify server session in background
        loginOwnerWithServer('bhuttu4770', '4770').catch(() => {});
