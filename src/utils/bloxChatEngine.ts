...
/**
 * Searches obtainment guides for matching items or keywords
 */
function findMatchingObtainmentGuide(query: string): ItemObtainmentGuide | null {
  const clean = query.replace(/[?.,!/\\-]/g, ' ').toLowerCase();

  // 1. Direct match on item ID or Name
  for (const item of ALL_OBTAINMENT_DATA) {
    const nameLower = item.name.toLowerCase();
    const idClean = item.id.replace(/-/g, ' ');

    if (clean.includes(nameLower) || clean.includes(idClean)) {
      return item;
    }
  }

  // 2. Specialized keyword mappings (Strict multi-word or distinct terms)
...
function formatObtainmentResponse(guide: ItemObtainmentGuide): string {
  ...
  return `${guide.icon} **${guide.name} Master Obtainment Dossier:**\n\n` +
...
function formatSeaLocationResponse(loc: SeaLocation): string {
  return `🗺️ **${loc.name} (Sea ${loc.sea}) Island Dossier:**\n\n` +
...
function formatRaidResponse(raid: RaidInfo): string {
  return `🔮 **${raid.fruitName} Raid & Awakening Dossier:**\n\n` +
...
function findMatchingFruitItem(query: string, allItems: FruitItem[]): FruitItem | null {
  ...
  // Common English words that shouldn't trigger fruit matching without Blox Fruits context
  const ambiguousFruitNames = new Set([
    'spin', 'love', 'chop', 'spring', 'bomb', 'smoke', 'spike', 'flame', 'ice', 
    'sand', 'dark', 'light', 'rubber', 'barrier', 'ghost', 'sound', 'pain', 'portal', 'control', 'shadow'
  ]);
  ...
  for (const item of allItems) {
    ...
    if (clean === nameLower || clean === idLower || clean === `${nameLower} fruit` || clean === `perm ${nameLower}`) {
      return item;
    }

    // 2. Ambiguous names require fruit trading/game context
    if (ambiguousFruitNames.has(nameLower)) {
      const hasFruitContext =
        ...
        clean.includes('m1') ||
        clean.includes('beli') ||
        clean.includes('awakening') ||
        clean.includes('demand');

      if (words.includes(nameLower) && hasFruitContext) {
        return item;
      }
    } else {
      // 3. Unambiguous names (e.g. Kitsune, Leopard, Dragon, Dough, Venom, Spirit, Blizzard, T-Rex, Mammoth, Buddha, Gravity, Rumble, Magma, Quake, Falcon)
      if (words.includes(nameLower) || clean.includes(nameLower)) {
        return item;
      }
    }
  }
...
function formatFruitItemResponse(item: FruitItem, query: string): string {
  ...
  return `${item.imageEmoji || '🍎'} **${item.name} Master Dossier:**\n\n` +
...
function detectAndHandleMutationLabQueries(query: string): string | null {
  ...
  const isMutationQuery = 
    ...
    clean.includes('fall') || 
    clean.includes('falling') || 
    clean.includes('rate') || 
    clean.includes('rating') || 
    clean.includes('fusion') || 
    clean.includes('hybrid') || 
    clean.includes('gacha') || 
    clean.includes('awakening') || 
    clean.includes('lab') ||
    clean.includes('chance');
